...
const activeTab = ref<'basic' | 'recommendation'>('basic')
...
function addTag(idx: number) {
  const tag = (tagInputs.value[idx] || '').trim()
  if (!tag) return
  if (!config.value.products[idx].tags) config.value.products[idx].tags = []
  if (!config.value.products[idx].tags.includes(tag)) {
    config.value.products[idx].tags.push(tag)
  }
  tagInputs.value[idx] = ''
}

function removeTag(idx: number, tagIdx: number) {
  config.value.products[idx].tags.splice(tagIdx, 1)
}
    ...
    <main class="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <template v-else>
        ...
        <div class="flex border-b border-gray-200">
          ...
          <button
            class="px-5 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px"
            :class="activeTab === 'recommendation'
              ? 'text-indigo-600 border-indigo-500'
              : 'text-gray-500 border-transparent hover:text-gray-700 hover:border-gray-300'"
        ...
        <template v-if="activeTab === 'recommendation'">
          <section class="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
            <div>
              <h2 class="text-base font-semibold text-gray-900">興味度判定プロンプト</h2>
              <p class="text-sm text-gray-500 mt-1">
                ユーザーの興味度を判定する際のガイドラインです。システムプロンプトに注入されます。
              </p>
            </div>
            <textarea
              v-model="config.interest_prompt"
              rows="6"
              class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm font-mono leading-relaxed resize-y focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              placeholder="興味度判定のガイドラインを入力..."
            />
            <div class="w-48">
              <label class="block text-sm font-medium text-gray-700 mb-1">興味度閾値</label>
              <input
                v-model.number="config.interest_threshold"
                type="number"
                min="1"
                max="5"
                step="1"
                class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              />
              <p class="text-xs text-gray-400 mt-1">1〜5。この値以上で商品推薦フェーズに移行します</p>
            </div>
          </section>
          ...
          <section class="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
            <div class="flex items-start justify-between gap-3">
              <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                  おすすめ対象の商品を設定します。商品が1件以上あると興味判定と推薦機能が有効になります。
                </p>
              </div>
              <button
                type="button"
            ...
            >
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">タグ</label>
                <div class="flex flex-wrap gap-1 mb-2" v-if="product.tags?.length">
                  <span
                    v-for="(tag, tIdx) in product.tags"
                    :key="tIdx"
                    class="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full"
                  >
                    {{ tag }}
                    <button type="button" class="text-indigo-400 hover:text-indigo-700" @click="removeTag(idx, tIdx)">&times;</button>
                  </span>
                </div>
                <div class="flex gap-2">
                  <input
                    v-model="tagInputs[idx]"
                    type="text"
                    class="flex-1 border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder="タグを入力してEnterまたは追加"
                    @keydown.enter.prevent="addTag(idx)"
                  />
                  <button type="button" class="px-3 py-1.5 text-xs font-medium text-indigo-600 border border-indigo-200 rounded-md hover:bg-indigo-50" @click="addTag(idx)">追加</button>
                </div>
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">関連商品ID（カンマ区切り）</label>
                <input
                  :value="(product.related_product_ids || []).join(', ')"
                  type="text"
                  class="w-full border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  placeholder="h002, h003"
                  @change="product.related_product_ids = ($event.target as HTMLInputElement).value.split(',').map(s => s.trim()).filter(Boolean)"
                />
                <p class="text-xs text-gray-400 mt-1">推薦後に連続紹介する商品のIDを指定します</p>
              </div>
            </div>
          </section>
        </template>
